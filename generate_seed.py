...
for lid, linfo in leagues.items():
    ...
    for idx, club in enumerate(linfo['clubs']):
        name, short, slug, city, cap, pres, cash = club
        cid = f'c{lid[2]}00000-0000-0000-0000-{idx+1:012d}'
        sid = f'b{lid[2]}00000-0000-0000-0000-{idx+1:012d}'
        
        sql.append(f"INSERT INTO public.stadiums (id, name, city, capacity) VALUES ('{sid}', '{name} Stadium', '{city}', {cap});")
        sql.append(f"INSERT INTO public.clubs (id, name, short_name, slug, country, city, league_id, stadium_id, prestige, fan_loyalty, infrastructure, youth_academy, is_ai_controlled, primary_color, secondary_color, founded_year) VALUES ('{cid}', '{name}', '{short}', '{slug}', '{country}', '{city}', '{lid}', '{sid}', {pres}, 80, 80, 80, true, '#1a1a2e', '#16213e', 1900);")
        sql.append(f"INSERT INTO public.finances (club_id, cash_balance, transfer_budget) VALUES ('{cid}', {cash}, {cash // 2});")
        
        # generate 22 players
        for i in range(22):
            pos = random.choice(['GK', 'CB', 'LB', 'RB', 'CDM', 'CM', 'CAM', 'LM', 'RM', 'LW', 'RW', 'ST'])
            year = random.randint(1990, 2006)
            ovr = max(50, min(95, pres - 15 + random.randint(0, 20)))
            pname = f'Player {i+1} {short}'
            sql.append(f"SELECT seed_player('{pname}', '{pos}', {ovr}, '{country}', {year}, '{cid}');")

with open('seed_gen.sql', 'w', encoding='utf-8') as f:
    f.write('\n'.join(sql))
